...
export function translate (m, v) { // v 是个二维向量，即一个数组 [x, y]，向量 v 是用于表示 x y 轴方向移动的距离，及其可表示移动的方向
  let arr = [...m]
  arr[4] = arr[4] + v[0]
  arr[5] = arr[5] + v[1]
  return arr
...
export function scale (m, v) { // v 是个二维向量，即一个数组 [scaleX, scaleY]，向量 v 是用于表示 x y 方向的伸缩程度
  /**
   * 假如 X，Y 分别缩放（a, b）倍
   * X' = X * a
   * Y' = Y * b
